...
void Application::Display(void)
{
	...
	v3Start = m_stopsList[movementNode]; //start at the current route
	v3End = m_stopsList[(movementNode + 1) % m_stopsList.size()]; //end at route +1 (if overboard will restart from 0)
	//
	//
	//Get a timer
	...
	if (fPercentage >= 1.0f)
	{
		movementNode++; //go to the next route
		fTimer = m_pSystem->GetDeltaTime(uClock);//restart the clock
		movementNode %= m_stopsList.size();//make sure we are within boundries
	}
	// render the object
