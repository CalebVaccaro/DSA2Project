...
void Application::InitVariables(void)
{
	...
	//InitProjectionArray for Cameras
	InitCameraProjections();
}
void Simplex::Application::InitCameraProjections(void)
{
	for (uint i = 0; i < 7; i++)
	{
		Projections[i] = new vector3(x,y,z);
		x += 2;
		y += 2;
	}
}
...
void Application::Display(void)
{
	...
	default:
	case 1:
		m_pCamera->ResetCamera(Projections[0]);
		break;
	case 2:
		m_pCamera->ResetCamera(Projections[1]);
		break;
	case 3:
		m_pCamera->ResetCamera(Projections[2]);
		break;
	case 4:
		m_pCamera->ResetCamera(Projections[3]);
		break;
	case 5:
		m_pCamera->ResetCamera(Projections[4]);
		break;
	case 6:
		m_pCamera->ResetCamera(Projections[5]);
		break;
	case 7:
		m_pCamera->ResetCamera(Projections[6]);
		break;
	}
